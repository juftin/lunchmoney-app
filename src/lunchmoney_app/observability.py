...
class MetricsRegistry:
    ...
    def render(self) -> str:
        ...
        lines = [
            "# HELP lunchmoney_app_http_requests_total Completed HTTP requests.",
            "# TYPE lunchmoney_app_http_requests_total counter",
        ]
        lines.extend(
            _metric_line(
                "lunchmoney_app_http_requests_total",
        ...
        lines.extend(
            [
                "# HELP lunchmoney_app_mcp_requests_total Completed MCP transport requests.",
                "# TYPE lunchmoney_app_mcp_requests_total counter",
            ]
        )
        lines.extend(
            _metric_line(
                "lunchmoney_app_mcp_requests_total",
                {"status": str(status)},
                count,
            )
        ...
        lines.extend(
            [
                "# HELP lunchmoney_app_upstream_failures_total Lunch Money API failures.",
                "# TYPE lunchmoney_app_upstream_failures_total counter",
            ]
        )
        lines.extend(
            _metric_line(
                "lunchmoney_app_upstream_failures_total",
                {
                    "kind": kind,
        ...
        lines.extend(
            [
                "# HELP lunchmoney_app_sync_runs_total Completed synchronization runs.",
                "# TYPE lunchmoney_app_sync_runs_total counter",
            ]
        )
        lines.extend(
            _metric_line("lunchmoney_app_sync_runs_total", {"status": status}, count)
            for status, count in sorted(sync_runs.items())
        )
        lines.extend(
            [
                "# HELP lunchmoney_app_sync_duration_seconds Synchronization duration in seconds.",
                "# TYPE lunchmoney_app_sync_duration_seconds summary",
            ]
        )
        for status, (count, total) in sorted(sync_durations.items()):
            lines.append(
                _metric_line(
                    "lunchmoney_app_sync_duration_seconds_count",
                    {"status": status},
                    count,
                )
            )
            lines.append(
                _metric_line(
                    "lunchmoney_app_sync_duration_seconds_sum",
                    {"status": status},
                    total,
                )
            )
        lines.extend(
            [
                "# HELP lunchmoney_app_cache_last_successful_sync_timestamp_seconds Unix timestamp of the last successful cache refresh.",
                "# TYPE lunchmoney_app_cache_last_successful_sync_timestamp_seconds gauge",
            ]
        )
        if cache_timestamp is not None:
            lines.append(
                _metric_line(
                    "lunchmoney_app_cache_last_successful_sync_timestamp_seconds",
                    {},
                    cache_timestamp,
                )
            )
